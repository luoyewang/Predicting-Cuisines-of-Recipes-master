...
M = [10, 20, 40, 60, 80, 100, 120, 140, 160, 200]
...
def concatenate_cuisine_vec(df_pca_data, cuisine_vec, current_m):
    df_pca_data_concatenated = pd.DataFrame.copy(df_pca_data, deep=True)
    for i in range(current_m):
        df_pca_data_concatenated = pd.concat([df_pca_data_concatenated, pd.DataFrame(cuisine_vec)], axis=0)
    return df_pca_data_concatenated
...
if __name__ == '__main__':
    ...
    for current_m in M:
        ...
        df_pca_data = pd.DataFrame(new_data.T)
        ...
        df_pca_concatenated = concatenate_cuisine_vec(df_pca_data, cuisine_vec, current_m)
        np.save('base_data_for_pca_PMI_concatenated.npy', df_pca_concatenated)

        # start = time.process_time()
        for current_similarity_method in similarity_method:
            S = SIM_matrix(R=df_pca_concatenated, similarity_method=current_similarity_method, current_mode=current_mode)
            ...
            with open('./results/after_concatenated_results.txt', 'a') as f:
