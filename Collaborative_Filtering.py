...
if __name__ == '__main__':
    ...
    np.save('base_data_for_pca_PMI.npy', df_renamed_rev_train)

    for current_mode in mode:
        if current_mode == 'PCA':
            pca = KernelPCA(n_components=161, kernel='cosine')
        ...
        for current_similarity_method in similarity_method:
            ...
            for current_k in K:
                recall_test, mean_rank_test, median_rank_test = test_f(
                    df_renamed_rev_test, test_index, N, current_k, S)
                ...
                with open('./results/baseline_results.txt', 'a') as f:
